...
module.exports = {
    debug: function (roomNumber) {
        var mode = true;
        var config = getCache("roomconfig" + roomNumber);
        console.log(config.roles);

        for (var i = 0; i < 9; i++) {
            var item = config.roles[i];
            if (item.role === 'wolf' && mode) {
                mode = false;
            } else {
                this.sitDown(roomNumber, i + 1);
                this.getRole(roomNumber, i + 1);
            }
        }
    },
    ...
    getActionList: function (roomNumber, seatNumber) {
        ...
        var progress = getCache("roomprogress" + roomNumber);
        var steps = getCache("roomsteps" + roomNumber);
        var role = config.roles[seatNumber - 1].role;
