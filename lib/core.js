...
var preDefinedConfig = {
    ...
    p12: {
        total: 12,
        roles: ["seer", "witch", "guard", "idiot", "villager", "villager", "villager", "villager", "wolf", "wolf", "wolf", "wolf"],
        vicDead: {
            mortal: 4,
            god: 4,
            wolf: 4
        ...
        progress: ["night", "wolf", "witch", "seer", "guard", "day"]
    }
};
        ...
        if (debug) {
            ...
            if (config.total === 12) {
                roles.forEach(function (role) {
                    ...
                    if (role.role === 'guard') {
                        guard = role.seatNumber;
                    }
                });
                setTimeout(function () {
                    that.wolfKill(roomNumber, guard);
                }, 1000);
                setTimeout(function () {
                    that.actionWitch(roomNumber, witch, [guard, witch]);
                }, 2000);
                setTimeout(function () {
                    that.getRoleSeer(roomNumber, 1);
                }, 3000);
